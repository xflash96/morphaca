...
void WriteMesh(Mat& ctrl_points, Vec2f offset, int side_n);
...
int main( int argc, char *argv[] )
{
 	...
 	for( int t = 0 ; t < T ; t++ )
	{
		...
		char s[20];
		imwrite("auto.jpg", (morph+aligned_dst)/2*255);
					 
		time_t now = time(0);
		fprintf(stderr, "t = %d\n", (int)(now-st));
	}
	WriteMesh(ctrl_points, translation, side_n);

	return 0;
}

void WriteLine(FILE* fp, Vec2f _P, int id, int to)
{
	fprintf(fp, "0 0 %f %f %d 0 %d\n", _P.val[0], _P.val[1], id, to);
}
void WriteMesh(Mat& ctrl_points, Vec2f offset, int side_n)
{
	FILE *fsrc = fopen("src.asf", "w");
	FILE *fdst = fopen("dst.asf", "w");

	fprintf(fsrc, "%d\n", 2*side_n*(side_n-1));
	fprintf(fdst, "%d\n", 2*side_n*(side_n-1));
	int extidx = side_n*side_n;
	Vec2f odst, osrc;
	if(offset.val[0]<0)
		osrc.val[0] = -offset.val[0], odst.val[0] = 0;
	else
		osrc.val[0] = 0, odst.val[0] = offset.val[0];
	if(offset.val[1]<0)
		osrc.val[1] = -offset.val[1], odst.val[1] = 0;
	else
		osrc.val[1] = 0, odst.val[1] = offset.val[1];
	...
	for(int j=0; j<side_n; j++){
		...
		if( j+1<side_n){
			WriteLine(fdst, P+odst, idx, idx+1);
			WriteLine(fsrc, _P+osrc, idx, idx+1);
			existed = 1;
		}
		if( i+1<side_n ){
			int tidx=idx;
			if(existed)
				tidx =extidx++; 
			WriteLine(fdst, P+odst, tidx, idx+side_n);
			WriteLine(fsrc, _P+osrc, tidx, idx+side_n);
		}
	}
	}
	fclose(fsrc);
	...
	for( int i=-range; i<=range; i++ ){
		for( int j=-range; j<=range; j++){
			...
			if( diff < min_diff ){
				aligned_src = src;
				aligned_dst = dst;
				translation = Vec2f(di/img_src.rows, dj/img_src.cols);
				min_diff = diff;
